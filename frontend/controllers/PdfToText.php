...
/**************************************************************************************************************
    ...
    HISTORY
    [Version : 1.3.2]	[Date : 2016/11/06]     [Author : CV]
			...
			for  ( $i = 0, $count = count ( $matches [ 'keyword' ] ) ; $i  <  $count ; $i ++ )
			   {
				...
				$value		=  $this -> __extract_chars_from_block ( $value ) ;
			...
			if  ( isset ( $this -> TemplateObjects [ $template_object ] ) )
			   {
				...
				$replacements []	=  $this -> TemplateObjects [ $template_object ] ;
			    }

			// $objects_seen is here to prevent infinite recursion on PDF files generated by psychopathic software 
			// Such a situation should never happen
			if  ( isset ( $this -> PageContents [ $template_object ] ) &&  ! in_array ( $template_object, $objects_seen ) )
			   {
				$objects_seen []	=  $template_object ;
				$this -> __get_replacements ( $this -> PageContents [ $template_object ], $searches, $replacements ) ;
			    }
		    }
	    }
