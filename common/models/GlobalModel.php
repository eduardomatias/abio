...
class GlobalModel extends ActiveRecord 
{
    ...
    private function _saveMultiple($dados, $model = null, $relacao = null) {
        try {
    ...
    private function _saveRelated($dados, $relacao) {
        try {
            ...
            if ($modelPai->save()) {
                ...
                $idValor = $modelPai->{$id};
                $this->_saveMultiple($dados[1], null, [array_keys($relacao)[0], array_values($relacao)[0], $idValor]);
                /* ------------------------------------------------ */
            }
        } catch (\Exception $e) {
            throw $e;
    ...
    private function saveRelatedAndMultiple($tipo, $dados, $model, $relacao, $transacao) {
        ...
        try {
            ...
            if ($tipo == 1) {
                $this->_saveRelated($dados, $relacao);
            } else {
                $this->_saveMultiple($dados, $model, null);
            }
     ...
     */
    public function saveRelated($dados, $relacao, $transacao = true) {
        return $this->saveRelatedAndMultiple(1, $dados, null, $relacao, $transacao);
    }
     ...
     * @return true|Exception
     */
    public function saveMultiple($dados, $model, $transacao = true) {
        return $this->saveRelatedAndMultiple(2, $dados, $model, null, $transacao);
    }
